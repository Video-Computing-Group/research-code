function [ ds ] = CrossValidate_multi_basic(ds, learn_algs, X, params)
...
%     idxtrain=1:25;
   ...
   %%
    ...
    [Idx_A ,Idx_B,La1]= completeind2(X.train.asmat,X.train.Lmat,1:length(idxa),1:length(idxb),idxa,idxb);
...
%     end
   ...
   if(length(find(La1==1))>1) 
    ...
    for nameCounter=1:length(names)       
        ...
        [~,acc]= nn1clf((XX_te(:,X.test.idxa)),(XX_te(:,X.test.idxb)),X.test.inda,X.test.indb,ds(c).(names{nameCounter}).M);
      acc(1)
        fprintf('... done \n');
        ds.accuracy=acc;
    end
