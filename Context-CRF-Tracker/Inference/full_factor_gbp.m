function [bel, cflag, factorBels] = factor_gbp(numNodes,localPot,factorPot,varargin)
...
if (isempty(gbp.counting_node)==1)
    gbp.counting_node = zeros(1,numNodes);
    if (gbp.trw == 0)
...
if nargout>2
	[bel, cflag, factorBels] = c_full_factor_gbp(gbp.regions,numNodes,localPot,factorPot,maxIter,...
		gbp.sumOrMax,gbp.alpha,gbp.trw,gbp.counting_node,gbp.full,gbp.initMsg,temperature);
else
	[bel, cflag] = c_full_factor_gbp(gbp.regions,numNodes,localPot,factorPot,maxIter,...
		gbp.sumOrMax,gbp.alpha,gbp.trw,gbp.counting_node,gbp.full,gbp.initMsg,temperature);
end
